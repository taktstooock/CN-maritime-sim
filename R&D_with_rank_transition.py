...
class CustomAgent(Agent):
    def __init__(self, i):
        super().__init__(i)
        self.n_green = 0
        self.investment_cutoff = 2050  # 研究投資は2050年まで
        self.start_year = 2020  # エージェントの開始年を設定

        # 初期投資タイプの割り当て
        self.investment_type = random.choice(['Type1', 'Type2', 'Type3'])
        self.update_investment_properties()

    def update_investment_properties(self):
        """
        投資タイプに応じて、研究投資率とグリーン船の使用開始年を更新
        """
    ...
    def upgrade_investment_type(self, year):
        """
        5年経過後に Type2 → Type1, 10年経過後に Type3 → Type2 へ変更
        """
        years_elapsed = year - self.start_year

        if self.investment_type == 'Type2' and years_elapsed >= 25:
            self.investment_type = 'Type1'
            self.update_investment_properties()
            print(f"Agent {self.ind}: Type2 → Type1 at year {year}")

        elif self.investment_type == 'Type3' and years_elapsed >= 15:
            self.investment_type = 'Type2'
            self.update_investment_properties()
            print(f"Agent {self.ind}: Type3 → Type2 at year {year}")
    ...
    def trade(self, env, other_agents, year):
        ...
        - `Type2` は 5年経過後に `Type1` になる
        - `Type3` は 10年経過後に `Type2` になる
        """
        self.upgrade_investment_type(year)  # 投資タイプの変更をチェック
        ...
        for diff_oil in range(-test_case, test_case + 1, 2):
            ...
            for i in range(future_years):
                ...
                adjusted_pv_green = env.pv_green * 0.7 if self.investment_type == 'Type1' and 2027 <= year <= 2035 else env.pv_green
